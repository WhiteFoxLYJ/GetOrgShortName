...
//	int secMinIndex = 300000;
	int sampleCount = 4355;// 参数30需要根据数据量适当变化，一般100000的时候取200左右，300000取500左右
							// 关键字频率
...
//			System.out.println(line);
	...
	public void CreateCsvByDict(String csvPath, String path , String ShengShipath) throws Exception {
		// 根据字典词频输出csv文件
...
//				}
				...
				if(cut_name.get(i).equals("\"")||cut_name.get(i).equals("（")||cut_name.get(i).equals("(")||cut_name.get(i).equals("）")||cut_name.get(i).equals(")")){
					cut_name.remove(i);
					i--;
				}
...
//				System.out.println("each_word="+each_word);
				...
				for(String SHQline: SHQlist){
					...
					if(each_word.equals(SHQline)||each_word.equals(SHQline+"省")||each_word.equals(SHQline+"市")){
						lessSSQX = each_word;
...
//			linklist.add(lessSSQX);
			for(String each_word : wordlist){
				if((each_word.length()==1&&each_word!="（"&&each_word!="）")||each_word==minFrequencyWord||each_word==secMinFrequencyWord||(WordDict.get(each_word)<sampleCount&&each_word!=shengword&&each_word!=shiword&&each_word!=xianword&&each_word!=quword&&each_word!=zhenword&&each_word!=lessSSQX)){
...
//			System.out.println("enterword==="+enterword+"//enterword.length === " + enterword.length());
...
//					System.out.println("word.substring("+i+","+ (i+1)+")="+enterword.substring(i, i+1));
...
//						System.out.println("word.substring("+i+","+ (i+1)+")="+word.substring(i, i+1));
...
//				System.out.println("linklist="+linklist);
	 ...
	 * @throws IOException
	 */
	public void AddNewLine(String path, String id, String maxname, String minname,String qu_name,String shi_qu_name) throws IOException {
		// 向csv文件中添加一条新的数据
		File csv = new File(path); // CSV数据文件
		OutputStreamWriter osw = new OutputStreamWriter(new FileOutputStream(csv),"GBK");
...
//		BufferedWriter bw = new BufferedWriter(new FileWriter(csv, true)); // 附加
	...
	public static void main(String[] args) throws Exception {
		...
		csvTest.CreateCsvFile(txtpath, tilelist);// 创建csv文件
		
		csvTest.CreateDic(csvpath);
		csvTest.CreateCsvByDict(csvpath, txtpath, Shengshipath);
